...
const ImageGalleryPage = () => {
  ...
  const fetchImages = async () => {
    try {
      let res = await apiUrl.get(
        `/api/userpost/getallpost?page=${page}&limit=7&category=${
          category === null ? "" : category
        }&post_title=${post_title === null ? "" : post_title}&location=${
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <div
        style={{ position: "absolute", top: "0px", left: "0px", color: "blue" }}
        onClick={hangleHome}
      >
        <button
          style={{
            borderRadius: "12px", // Round the corners
            backgroundColor: "blue", // Fill the button with blue color
